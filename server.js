...
console.log('🕋 ULTIMATE BALANCE FIX - FINAL VERSION');
console.log('📦 Version: 37.0.0');
console.log('✅ CORRECT BALANCE FROM EXNESS');
console.log('✅ SCANS EVERY POSSIBLE FIELD');
...
async function fetchRealBalance(accountId) {
    try {
        ...
        // ===== ULTIMATE BALANCE EXTRACTION - SCANS EVERY FIELD =====
        let balance = 0;
        let currency = accountInfo.currency || accountInfo.Currency || 'USD';
        let foundField = null;

        // ALL possible field names that could contain balance
        const allFields = [
            // Standard fields
            ...
            // Compound fields
            'balance_amount', 'BalanceAmount', 'balanceAmount',
            'total_balance', 'TotalBalance', 'totalBalance',
            'account_balance', 'AccountBalance', 'accountBalance',
            'client_balance', 'ClientBalance', 'clientBalance',
            'cash_balance', 'CashBalance', 'cashBalance',
            'free_balance', 'FreeBalance', 'freeBalance',
            'net_balance', 'NetBalance', 'netBalance',
            'asset_value', 'AssetValue', 'assetValue',
        ...
        // Method 1: Check all known field names
        ...
        // Method 2: Scan EVERY single field for ANY positive number
        if (balance === 0) {
            ...
            const balanceKeywords = [
                ...
                'capital', 'avail', 'usable', 'client', 'account', 'profit', 
                'pnl', 'unrealized', 'realized', 'trading'
            ];
        ...
        // Method 3: Take the LARGEST positive number
        if (balance === 0) {
        ...
        console.log(`✅ Found in field: ${foundField || 'Not found - using 0'}`);
...
async function getSimpleSignal(symbol, accountId) {
    try {
        ...
        const confidence = 0.6 + (Math.abs(currentPrice - ma20) / ma20) * 5;
...
class TradingEngine {
    ...
    async start() {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            if (this.firstTradeOpened) break;
            console.log(`🔄 First trade attempt ${attempt}/3...`);
        ...
        this.analysisInterval = setInterval(async () => {
            ...
            if (this.openPositions.length < this.maxConcurrentTrades) {
                for (const symbol of this.config.tradingPairs) {
                    ...
                    try {
                        const signal = await getSimpleSignal(symbol, this.accountId);
                        if (signal.action) {
    ...
    async executeTrade(symbol, side, signal) {
        try {
            ...
            const positionSize = Math.min(balance / 10, balance * 0.15);
            ...
            console.log(`📈 EXECUTING ${side} on ${symbol} at $${entryPrice} (NO LEVERAGE)`);
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🕋 ULTIMATE BALANCE FIX - FINAL VERSION`);
